...
namespace ProjectManagementSystem.Controllers
{
	public class MissionController : Controller
	{
		...
		public async Task<IActionResult> Index()
		{
			...
			{
				UntreatedMissions = missions.Where(a => a.Status.Status == "未处理"),
				ProcessOnMissions = missions.Where(a => a.Status.Status == "进行中"),
				FinishedMissions = missions.Where(a => a.Status.Status == "已完成"),
			};
			return View(model);
		}
		[HttpPost]
		public async Task<IActionResult> OnSelectOne(int id)
		{
			...
			MissionIndexViewModel model = new MissionIndexViewModel
			{
				...
				ProcessOnMissions = missions.Where(a => a.Status.Status == "进行中"),
				FinishedMissions = missions.Where(a => a.Status.Status == "已完成"),
				CurrentMission = await dataDbContext.Missions.FirstOrDefaultAsync(a => a.Id == id),
			};
			return View(model);
		}
